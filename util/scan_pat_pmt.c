...
#include "scan_descriptors.h"

static const char * demux_devname = "/dev/dvb/adapter0/demux0";
static int opt_verbosity = 2; // used within macro dprintf()
...
int scan_pat_pmt(const char * demux, scan_services_t * srv, int srv_cnt) {
  struct section_buf sect;
  int result = 0;
  ...
  for(int idx = 0; idx < srv_cnt; ++idx) {
    alloc_service(srv[idx].service_id);
    }

  setup_filter(&sect, demux_devname, PID_PAT, TABLE_PAT, -1, 1, 0, 0);
  add_filter(&sect);
  ...
  for(struct service * s = current_services->first; s; s = s->next) {
    //printf("%d: video:%d teletext:%d\n", s->service_id, s->video_pid, s->teletext_pid);
    for(int idx = 0; idx < srv_cnt; ++idx) {
      if (srv[idx].service_id == s->service_id) {
        srv[idx].ttx_pid = s->teletext_pid;
        result += 1;
        }
      }
    }
